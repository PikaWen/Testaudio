...
public class Database
{
    ...
    private SqliteTransaction dbSqlTransaction = null;//資料大量插入時使用
    ...
    public void databaseConnection(string databaseName)
    {
        try
        {
            string connectString;
                        ...
                        connectString = "URI=file:" + Application.dataPath + "/Plugins/Android/assets" + "/" + databaseName;
                        dbSqlConnection = new SqliteConnection(connectString);
            ...
            #elif UNITY_ANDROID
                              //若是在Android平台
                            connectString = "URI=file:" + Application.persistentDataPath + "/" + databaseName;
    ...
    /// <summary>
    /// 若要一次插入大筆資料，請依序執行insertBigDataInitialize、insertBigDataInto、insertBigDataWrite
    /// </summary>
    public void insertBigDataInitialize()
    {
        dbSqlCommand = dbSqlConnection.CreateCommand();
        dbSqlTransaction = dbSqlConnection.BeginTransaction(System.Data.IsolationLevel.Serializable);
    }
    ...
    public void insertBigDataInto(string tableName, string[] value)
    {
        ...
        query.Append(")");
        dbSqlCommand.CommandText = query.ToString();
    ...
    /// 將資料寫入資料表內
    /// </summary>
    public void insertBigDataWrite()
    {
        try
        {
            dbSqlTransaction.Commit();
            dbSqlTransaction = null;
        }
        catch (Exception e)
        {
            dbSqlTransaction.Rollback();
            throw e;
        }
    }
    ...
    /// <returns></returns>
    public SqliteDataReader searchAccordData(string tableName, string SongName, string SongN, string column)
    {

        string query = "SELECT Distinct * FROM " + tableName + " WHERE " + SongName + "='" + SongN + "' " + column ;
